...
async function chargerAnimesAnciens() {
    try {

        const reponse = await fetch('https://api.jikan.moe/v4/top/anime');
        const donnees = await reponse.json();
        const container = document.getElementById('anime-container');
        container.innerHTML = ''; 

        // Affichage des 12 premiers résultats 
        donnees.data.slice(0, 12).forEach(anime => {
            const html = `
        ...
        document.getElementById('anime-container').innerHTML = "Impossible de charger les animés.";
    }
}

// On lance la fonction au chargement de la page
chargerAnimesAnciens();